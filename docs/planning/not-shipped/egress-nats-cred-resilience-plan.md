...
- **Known limitation: an initial-connect auth failure is invisible to self-heal.** An egress gateway `Fatal`s on a *failed initial* NATS connect, and its `/healthz` only starts once the bus is up — so a gateway that crash-loops on an initial auth rejection never reports `auth-failed`, and Phase 4's supervisor can't heal it (surfaced by the Phase 6 code review). Mitigated by Phase 6 (default ON) rewriting the creds file within seconds of a rotation, and by a crash-loop being visibly unhealthy anyway (unlike the silent case this plan targets). Documented in-code in `egress-gateway/cmd/main.go`; the follow-up fix is to start the health server before the initial connect and use `RetryOnFailedConnect` instead of `Fatal`.
