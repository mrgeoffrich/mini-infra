...
description: Turns a brainstorming session — a scratch markdown file, in-conversation ideation, or inline notes — into a phased planning document at `docs/planning/not-shipped/<slug>-plan.md` that conforms to `docs/planning/PLANNING.md`. Drives a rubric-driven phase split rather than dumping the brainstorm into a template verbatim. Each phase must pass seven checks — Concrete (noun-form deliverables), Testable (one-sentence Done-when), Isolated (one shippable PR with no scaffolding leaking forward), One-concern (single component or capability slice), Reversibility-classifiable (`safe`/`feature-flagged`/`forward-only`/`destructive` without weasel words), UI-extractable (every user-visible change tagged `[design needed]` or `[no design]`, never `TBD`), Verify-in-prod-statable (production signal that confirms the *outcome*, not just artifact existence). Surfaces failing phases with concrete split suggestions and iterates until every phase passes or the user explicitly waives a check; waivers are logged as a one-line note in §1 Background so future readers know the rubric was bypassed. Applies pre-emptive split heuristics for known anti-patterns — framework-plus-first-user, polish-catch-all, new-connected-service-plus-first-consumer, schema-and-API-and-UI-in-one-phase, pool-support-bolted-onto-feature, docs-padded-onto-feature — so the first proposal is already close before the rubric runs. Can also re-phase an existing plan doc whose phasing is broken (too big, untestable Done-when, "polish" catch-all phases). Use this skill whenever the user says "turn this brainstorm into a plan", "draft a planning doc from these notes", "split this idea into phases", "phase this out for me", "write up a plan for X", "fix the phasing of this plan", "this plan's phases are too big — re-split them", or has clearly been ideating in-session and asks for a plan doc. Do NOT use for ad-hoc one-liner planning, or for plans that are already well-phased and only need wording tweaks (edit the doc directly).
---

# Brainstorm to Plan

You're turning a brainstorming session into a phased planning document under `docs/planning/not-shipped/`. Each phase is scoped to ship as one PR, executed via a worktree (`/setup-worktree` → code → `/review` → `/finish-worktree`). The phased rollout is the load-bearing artifact — get the splits right and each phase is a clean, reviewable unit of work; get them wrong (phases too big, deliverables vague, Done-when untestable) and whoever implements it wastes time figuring out what "done" means.

Plans are **scoping** documents, not implementations. You write goals, deliverables in noun form, reversibility classifiers, UI changes, Done-when, and Verify-in-prod — not file-by-file change lists. Source-code touchpoints get worked out at implementation time, not planning time.
...
### Mode D — Re-phase an existing plan doc

The user points at an existing plan doc whose phasing is broken — phases too big, Done-when untestable, "polish" catch-all phases — and wants it re-split.

In this mode, the existing plan **is the brainstorming input**. Read it like a scratch doc, then run it through the same rubric-driven splitting as any other input.

**Confirm the mode with the user before proceeding.** A one-liner is enough: "Reading `<path>` as a Mode A scratch file" or "Re-phasing `<path>` (Mode D)."
...
- [`docs/planning/PLANNING.md`](docs/planning/PLANNING.md) — the mechanical format the plan must conform to. Section numbering, the six-required-parts-per-phase rule, "What not to write."
...
| **Design / mockup work split out as its own phase** | Don't write standalone `Phase N — Design: …` phases. Keep design items as `[design needed]` tags inline on the impl phase — whoever picks up the phase handles design exploration as a prerequisite step before coding. |
...
Optional phases still go through the rubric. Mark them clearly in the phase tracking list so whoever picks up the plan knows they're not blocking the main line.
...
**6. UI-extractable.** Pass: every user-visible change is a bullet tagged `[design needed]` or `[no design]`, or the literal word `none`. Never `TBD`. Reject: "Phase N's UI changes include 'TBD' or untagged items. Either tag each item explicitly (a clean [no design] is fine) or split out the UI-bearing slice as its own phase so the design ask is scoped." (Note: don't pre-split design out as its own phase; just tag the items — design exploration happens as a step within the impl phase.)
...
Apply the user's parallel edges as `[blocks-by: N, M]` brackets in the phase tracking list. If the user says "no, sequential", keep the default and don't render brackets at all (per [PLANNING.md](docs/planning/PLANNING.md) — bracket-free means strict sequential).
...
<one-line note that this is a manual checklist, updated as work lands>
...
### Re-phase mode (Mode D)

When re-phasing an existing plan doc, the output replaces the old phased-rollout and phase-tracking sections wholesale — there's no ID bookkeeping to preserve, just the new split. Summarise what changed before writing:

```
Re-phasing `<path>`.

Old phases (N): <old titles, one line>
New phases (M): <new titles, one line, noting which are splits of which old phase>
...
Leave the file untracked / staged. The user reviews it, then implements each phase in order (e.g. via `/setup-worktree` per phase).
...
<for re-phase mode:>
  - Phases carried over unchanged: <count>
  - Phases newly split: <count>
  - Phases dropped: <count>

Next step: read through the plan, then implement Phase 1 (e.g. via /setup-worktree).
```
...
- **Plans are scoping documents, not implementations.** No file-by-file change lists, no "edit foo.ts to do X." Source-code touchpoints get worked out at implementation time — never preempt that.
...
- **Re-phase mode surfaces dropped phases, never silently discards their context.** If a phase is merged or dropped during re-splitting, call it out in the final summary — it may have carried decisions or rationale worth preserving elsewhere.
