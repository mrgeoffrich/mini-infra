...
5. Using markdown format, output this plan into a markdown file.
6. Post a comment in that issue using that markdown file as the content.
7. Delete that markdown file as its no longer required.
8. Notify the user that this is complete by running `pushover-cli.exe send "message" -u https://github.com/xxx` and replace the message with a message that should be very short, 10 words maximum, indicating what has been done. After the -u add the link to the github issue.
