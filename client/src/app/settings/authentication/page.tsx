...
/**
 * Form component for editing auth settings. State is initialized from the
 * `settings` prop. The parent remounts this component (via a `key` tied to
 * the settings identity) whenever fresh settings arrive from the server, so
 * there's no need to sync state in an effect.
 */
...
export default function AuthenticationSettingsPage() {
  ...
  // Use a key derived from the settings content so the form remounts (and
  // re-initializes its local state) whenever the server-side settings change.
  const formKey = `${settings.googleOAuthEnabled}:${settings.googleClientId ?? ""}:${settings.hasGoogleClientSecret ? 1 : 0}`;

  return (
    <div className="space-y-6">
      <AuthSettingsForm key={formKey} settings={settings} />
    </div>
  );
}
