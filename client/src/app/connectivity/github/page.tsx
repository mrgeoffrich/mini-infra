          ...
          {!oauthAuthorized && showPatInput && (
            <div className="mt-3 space-y-2">
              <p className="text-xs text-muted-foreground">
                Generate the token on GitHub, then paste it below:
              </p>
            ...
            <div className="mt-3 space-y-2">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  onKeyDown={(e) => e.key === "Enter" && handleSaveAgentToken()}
                  className="font-mono text-sm h-9"
                />
                <Button
                  size="sm"
                  onClick={handleSaveAgentToken}
                  disabled={saveAgentToken.isPending || !agentTokenInput.trim()}
                >
                  {saveAgentToken.isPending ? (
                    <IconLoader2 className="mr-2 h-4 w-4 animate-spin" />
                  ) : null}
                  Save
                </Button>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => { setShowAgentTokenInput(false); setAgentTokenInput(""); }}
                >
                  Cancel
                </Button>
              </div>
