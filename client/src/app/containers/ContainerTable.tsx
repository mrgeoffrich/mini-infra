...
const CopyButton = React.memo(({ text, className = "h-6 w-6 p-0" }: { text: string; className?: string }) => {
  ...
  prevProps.text === nextProps.text && prevProps.className === nextProps.className
);

CopyButton.displayName = "CopyButton";

const ContainerNameCell = React.memo(({ name }: { name: string }) => (
...
const ContainerImageCell = React.memo(({ image, imageTag }: { image: string; imageTag: string }) => {
  const fullImage = React.useMemo(() => `${image}:${imageTag}`, [image, imageTag]);
  return (
  ...
  prevProps.image === nextProps.image && prevProps.imageTag === nextProps.imageTag
);

ContainerImageCell.displayName = "ContainerImageCell";

const ContainerIPCell = React.memo(({ ip }: { ip: string }) => {
  if (!ip) {
    return <span className="text-muted-foreground min-h-[2rem] flex items-center">-</span>;
  }
  return (
    <div className="flex items-center gap-2 min-h-[2rem]">
...
const ContainerPortsCell = React.memo(({ ports }: { ports: ContainerInfo["ports"] }) => {
  if (!ports.length) {
    return <span className="text-muted-foreground min-h-[2rem] flex items-center">No ports</span>;
  }
  return (
  ...
  return prevProps.ports.every((port, index) => {
    ...
    return port.public === nextPort.public && 
...
const ContainerDateCell = React.memo(({ createdAt }: { createdAt: string }) => {
  const date = React.useMemo(() => new Date(createdAt), [createdAt]);
  const formattedDate = React.useMemo(() => format(date, "MMM d, yyyy"), [date]);
...
const ContainerRow = React.memo(({ 
  ...
  <TableRow
    key={container.id}
    className="hover:bg-muted/50 h-16"
  >
...
export const ContainerTable = React.memo(function ContainerTable({
  ...
  const handleCreatedSort = React.useCallback(() => updateSort("createdAt"), [updateSort]);
  ...
  const columns: ColumnDef<ContainerInfo>[] = React.useMemo(() => [
    ...
    },
    {
      ...
      header: "Ports",
      cell: ({ row }) => (
        <ContainerPortsCell ports={row.getValue("ports")} />
      ),
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => <ContainerDateCell createdAt={row.getValue("createdAt")} />,
    },
  ...
  const totalPages = React.useMemo(() => Math.ceil(totalCount / limit), [totalCount, limit]);
  const startItem = React.useMemo(() => (page - 1) * limit + 1, [page, limit]);
  const endItem = React.useMemo(() => Math.min(page * limit, totalCount), [page, limit, totalCount]);

  // Fixed column widths to prevent layout shifts
  const getColumnWidth = React.useCallback((index: number) => {
    switch (index) {
      case 0: return 'w-[200px] min-w-[200px] max-w-[200px]'; // Container Name
      case 1: return 'w-[120px] min-w-[120px] max-w-[120px]'; // Status
      case 2: return 'w-[280px] min-w-[280px] max-w-[280px]'; // Image
      case 3: return 'w-[180px] min-w-[180px] max-w-[180px]'; // Ports
      case 4: return 'w-[140px] min-w-[140px] max-w-[140px]'; // IP Address
      case 5: return 'w-[160px] min-w-[160px] max-w-[160px]'; // Created
      default: return '';
    }
  }, []);
