...
def create_database_if_not_exists(postgres_host_opts, database_name):
    ...
    if database_exists(postgres_host_opts, database_name):
        ...
        try:
            quoted_db = shlex.quote(database_name)
            cmd = f'psql {postgres_host_opts} -c "CREATE DATABASE {quoted_db}"'
            run_command(cmd)
...
def restore(postgres_database,
    ...
    try:
        ...
        try:
            ...
            # Stream download to disk to avoid loading entire blob into memory
        ...
        if drop_public == "yes":
            ...
            cmd = f'psql {postgres_host_opts} -d {quoted_db} -c "drop schema public cascade; create schema public;"'
            run_command(cmd)
        ...
        if restore_method == 'psql':
            ...
            cmd = f'psql {postgres_host_opts} -d {quoted_db}'
            ...
            cmd = f'pg_restore {postgres_host_opts} -d {quoted_db} {shlex.quote(restore_file)}'
        ...
        elapsed = (error_time - restore_start_time).total_seconds()
        print(f"[{error_time.strftime('%Y-%m-%d %H:%M:%S')}] Unexpected error after {elapsed:.2f} seconds: {e}", file=sys.stderr)
