...
Python equivalent of backup.sh
"""
...
def backup(postgres_database,
    ...
    if backup_format == 'custom':
        ...
        pg_dump_cmd = f"pg_dump {postgres_host_opts} --format=custom --compress={compression_level} {postgres_database}"
        ...
        pg_dump_cmd = f"pg_dump {postgres_host_opts} --format=directory --compress={compression_level} --file=dump_dir {postgres_database}"
        dump_filename = "dump_dir"
        final_filename = "dump.tar.gz"
    elif backup_format == 'tar':
        # Tar format with built-in compression
        pg_dump_cmd = f"pg_dump {postgres_host_opts} --format=tar --compress={compression_level} {postgres_database}"
        dump_filename = "dump.tar"
        final_filename = "dump.tar"
    else:
        # Plain SQL format (default) - we'll handle gzip compression manually
        pg_dump_cmd = f"pg_dump {postgres_host_opts} {postgres_database}"
    ...
    try:
        if backup_format == 'plain':
            ...
            result = run_command(pg_dump_cmd)
    ...
    try:
        ...
        print(f"Connection string (masked): {azure_connection_string[:20]}...{azure_connection_string[-10:] if len(azure_connection_string) > 30 else azure_connection_string}", file=sys.stderr)
        sys.exit(1)
