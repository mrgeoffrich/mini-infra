...
### NATS — [services/nats/](src/services/nats/), [services/stacks/stack-nats-apply-orchestrator.ts](src/services/stacks/stack-nats-apply-orchestrator.ts)

Built-in message bus, deployed as a managed host-scoped `nats` stack that requires the `vault` host stack to be synced and bootstrapped (declared via the cross-stack `requires` block). The control plane is fully DB-backed: [nats-control-plane-service.ts](src/services/nats/nats-control-plane-service.ts) owns accounts, credential profiles, JetStream streams/consumers, and the `applyConfig()` + `applyJetStreamResources()` reconcile pipeline triggered by `POST /api/nats/apply`. [nats-config-renderer.ts](src/services/nats/nats-config-renderer.ts) renders the server config. [nats-key-manager.ts](src/services/nats/nats-key-manager.ts) handles NKey/operator key material (stored in Vault). [nats-credential-injector.ts](src/services/nats/nats-credential-injector.ts) injects `NATS_CREDS` / `NATS_URL` / `NATS_SIGNER_SEED` env vars into services that bind to a credential profile or signer.

App-level surface (roles, signers, subject prefix, cross-stack imports/exports) lives on `StackTemplateVersion` and is materialized into per-stack `NatsCredentialProfile` rows by [stack-nats-apply-orchestrator.ts](src/services/stacks/stack-nats-apply-orchestrator.ts) during stack apply. The orchestrator resolves `subjectPrefix` (default `app.<stack.id>`), prepends it to every role's publish/subscribe entries, auto-injects `_INBOX.>` per `inboxAuto`, and writes the resolved exports into `lastAppliedNatsSnapshot` for downstream consumers. Imports are scoped to the consumer's `environmentId` to prevent cross-environment subject collisions.

[nats-prefix-allowlist-service.ts](src/services/nats/nats-prefix-allowlist-service.ts) is the admin-only gate for non-default subject prefixes — CRUD-per-entry under `SystemSettings` category `nats-prefix-allowlist`, exposed at `/api/nats/prefix-allowlist`. Validation rejects wildcards, leading/trailing dots, `$SYS` prefixes, subject-tree overlaps (ancestor or descendant), and references to nonexistent template IDs. The allowlist router **must** mount before the catch-all `/api/nats` router — see [app-factory.ts](src/app-factory.ts) for the order.
