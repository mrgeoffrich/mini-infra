...
export const createDeploymentConfigSchema = z.object({
  applicationName: z.string()
    ...
    .regex(/^[a-zA-Z0-9_-]+$/, "Application name can only contain letters, numbers, hyphens, and underscores"),
...
export class DeploymentConfigService extends ConfigurationService {
  ...
  async createDeploymentConfig(
    ...
    try {
      ...
      if (existing) {
        throw new Error(`Deployment configuration for application '${request.applicationName}' already exists`);
      }
  ...
  async updateDeploymentConfig(
    ...
    try {
      ...
      if (request.applicationName && request.applicationName !== existing.applicationName) {
      ...
      if (request.applicationName) updateData.applicationName = request.applicationName;
      if (request.dockerImage) updateData.dockerImage = request.dockerImage;
      if (request.dockerRegistry !== undefined) updateData.dockerRegistry = request.dockerRegistry;
      if (request.containerConfig) updateData.containerConfig = request.containerConfig;
      if (request.healthCheckConfig) updateData.healthCheckConfig = request.healthCheckConfig;
      if (request.traefikConfig) updateData.traefikConfig = request.traefikConfig;
      if (request.rollbackConfig) updateData.rollbackConfig = request.rollbackConfig;
   ...
   */
  async deleteDeploymentConfig(configId: string, userId: string): Promise<void> {
    try {
   ...
   */
  validateDeploymentConfiguration(config: CreateDeploymentConfigRequest): DeploymentConfigValidationResult {
    ...
    if (!config.applicationName || config.applicationName.trim().length === 0) {
      ...
      errors.push({ field: "applicationName", message: "Application name can only contain letters, numbers, hyphens, and underscores" });
    } else if (config.applicationName.length > 100) {
      errors.push({ field: "applicationName", message: "Application name must be 100 characters or less" });
    }

    // Validate docker image
    if (!config.dockerImage || config.dockerImage.trim().length === 0) {
      errors.push({ field: "dockerImage", message: "Docker image is required" });
    }
    ...
    return {
      ...
      message: errors.length === 0 ? "Configuration is valid" : "Configuration has validation errors",
  ...
  private validateDeploymentConfigRequest(request: CreateDeploymentConfigRequest): void {
    try {
   ...
   */
  validateWithZod(data: CreateDeploymentConfigRequest): { isValid: boolean; errors?: string[] } {
    try {
   ...
   */
  validateUpdateWithZod(data: UpdateDeploymentConfigRequest): { isValid: boolean; errors?: string[] } {
    try {
  ...
  private validateContainerConfig(config: ContainerConfig, errors: { field: string; message: string }[]): void {
    if (!config.ports || !Array.isArray(config.ports)) {
      errors.push({ field: "containerConfig.ports", message: "Ports array is required" });
    } else {
      config.ports.forEach((port, index) => {
        if (!port.containerPort || port.containerPort < 1 || port.containerPort > 65535) {
          errors.push({ field: `containerConfig.ports[${index}].containerPort`, message: "Container port must be between 1 and 65535" });
        }
        if (port.hostPort && (port.hostPort < 1 || port.hostPort > 65535)) {
          errors.push({ field: `containerConfig.ports[${index}].hostPort`, message: "Host port must be between 1 and 65535" });
        }
      });
    }

    if (!config.volumes || !Array.isArray(config.volumes)) {
      errors.push({ field: "containerConfig.volumes", message: "Volumes array is required" });
    }

    if (!config.environment || !Array.isArray(config.environment)) {
      errors.push({ field: "containerConfig.environment", message: "Environment variables array is required" });
    }

    if (!config.labels || typeof config.labels !== 'object') {
      errors.push({ field: "containerConfig.labels", message: "Labels object is required" });
    }

    if (!config.networks || !Array.isArray(config.networks)) {
      errors.push({ field: "containerConfig.networks", message: "Networks array is required" });
    }
  }

  private validateHealthCheckConfig(config: HealthCheckConfig, errors: { field: string; message: string }[]): void {
    if (!config.endpoint || config.endpoint.trim().length === 0) {
      errors.push({ field: "healthCheckConfig.endpoint", message: "Health check endpoint is required" });
    }

    if (!config.method || !['GET', 'POST'].includes(config.method)) {
      errors.push({ field: "healthCheckConfig.method", message: "Method must be GET or POST" });
    }

    if (!config.expectedStatus || !Array.isArray(config.expectedStatus)) {
      errors.push({ field: "healthCheckConfig.expectedStatus", message: "Expected status codes array is required" });
    }

    if (!config.timeout || config.timeout < 1000) {
      errors.push({ field: "healthCheckConfig.timeout", message: "Timeout must be at least 1000ms" });
    }

    if (!config.retries || config.retries < 1) {
      errors.push({ field: "healthCheckConfig.retries", message: "Retries must be at least 1" });
    }

    if (!config.interval || config.interval < 1000) {
      errors.push({ field: "healthCheckConfig.interval", message: "Interval must be at least 1000ms" });
    }
  }

  private validateTraefikConfig(config: TraefikConfig, errors: { field: string; message: string }[]): void {
    if (!config.routerName || config.routerName.trim().length === 0) {
      errors.push({ field: "traefikConfig.routerName", message: "Router name is required" });
    }

    if (!config.serviceName || config.serviceName.trim().length === 0) {
      errors.push({ field: "traefikConfig.serviceName", message: "Service name is required" });
    }

    if (!config.rule || config.rule.trim().length === 0) {
      errors.push({ field: "traefikConfig.rule", message: "Routing rule is required" });
    }
  }

  private validateRollbackConfig(config: RollbackConfig, errors: { field: string; message: string }[]): void {
    if (config.enabled === undefined) {
      errors.push({ field: "rollbackConfig.enabled", message: "Rollback enabled flag is required" });
    }

    if (!config.maxWaitTime || config.maxWaitTime < 1000) {
      errors.push({ field: "rollbackConfig.maxWaitTime", message: "Max wait time must be at least 1000ms" });
    }

    if (config.keepOldContainer === undefined) {
      errors.push({ field: "rollbackConfig.keepOldContainer", message: "Keep old container flag is required" });
    }
  }
