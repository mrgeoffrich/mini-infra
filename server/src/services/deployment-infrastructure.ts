...
export class DeploymentInfrastructureService {
  ...
  async ensureDeploymentNetwork(
    ...
    try {
      ...
      if (networks.length > 0) {
        logger.info({ networkName, networkId: networks[0].Id }, "Deployment network already exists");
        return { success: true, networkId: networks[0].Id };
      }

      // Create the network
      logger.info({ networkName, networkDriver }, "Creating deployment network");
      ...
      logger.info({ networkName, networkId: network.id }, "Deployment network created successfully");
      return { success: true, networkId: network.id };

    } catch (error) {
      logger.error({ error, networkName, networkDriver }, "Failed to create deployment network");
      return { 
        success: false, 
        error: error instanceof Error ? error.message : "Unknown error creating network" 
      };
    }
  }
  ...
  async ensureTraefikContainer(config: {
    ...
    try {
      ...
      if (containers.length > 0) {
        logger.info({ containerId: containers[0].Id }, "Stopping existing Traefik container");
        const existingContainer = docker.getContainer(containers[0].Id);
        
        try {
          await existingContainer.stop();
        } catch (stopError) {
          logger.warn({ error: stopError }, "Failed to stop existing container (may already be stopped)");
        }
        
        await existingContainer.remove();
        logger.info({ containerId: containers[0].Id }, "Removed existing Traefik container");
      }

      // Ensure the network exists
      const networkResult = await this.ensureDeploymentNetwork(config.networkName);
      if (!networkResult.success) {
        return { success: false, error: `Failed to create network: ${networkResult.error}` };
      }
      ...
      const container = await docker.createContainer({
        ...
        HostConfig: {
          PortBindings: {
            [`${config.webPort}/tcp`]: [{ HostPort: config.webPort.toString() }],
            [`${config.dashboardPort}/tcp`]: [{ HostPort: config.dashboardPort.toString() }]
          },
          Binds: [
      ...
      return { 
        success: false, 
        error: error instanceof Error ? error.message : "Unknown error deploying Traefik" 
      };
    }
  }
  ...
  async getInfrastructureStatus(networkName: string): Promise<{
    ...
    traefikStatus: { exists: boolean; running: boolean; id?: string; error?: string };
  }> {
    try {
      const docker = await this.dockerService.getDockerInstance();
      ...
      const networkStatus = networks.length > 0 
      ...
      let traefikStatus: { exists: boolean; running: boolean; id?: string; error?: string };
      ...
      logger.error({ error, networkName }, "Failed to get infrastructure status");
      return {
        networkStatus: { 
          exists: false, 
   ...
   */
  async cleanupInfrastructure(networkName: string): Promise<{ success: boolean; error?: string }> {
    try {
      const docker = await this.dockerService.getDockerInstance();
      ...
      for (const containerInfo of containers) {
        ...
        try {
          ...
          logger.warn({ error: stopError, containerId: containerInfo.Id }, "Failed to stop container");
        }
        await container.remove();
        logger.info({ containerId: containerInfo.Id }, "Removed Traefik container");
      }
      ...
      for (const networkInfo of networks) {
        ...
        logger.info({ networkId: networkInfo.Id, networkName }, "Removed deployment network");
      }
