...
describe("InMemoryQueue", () => {
  ...
  describe("Concurrency", () => {
    ...
    test("should process multiple jobs concurrently", async () => {
      ...
      const firstThreeStartTimes = [
        startTimes[1],
        startTimes[2],
        startTimes[3],
      ];
      const maxStartTimeDiff =
        Math.max(...firstThreeStartTimes) - Math.min(...firstThreeStartTimes);
  ...
  describe("Event System", () => {
    ...
    test("should emit progress events", async () => {
      ...
      expect(progressEvents.map((e) => e.progress)).toEqual(
  ...
  describe("Job Management", () => {
    ...
    test("should remove pending jobs", async () => {
      ...
      await expect(queue.remove(job1.id)).rejects.toThrow(
    ...
    test("should handle queue closure gracefully", async () => {
      ...
      await expect(queue.add("test-job", {})).rejects.toThrow(
        "Queue is closed",
      );
    });
  });
  ...
  describe("Statistics", () => {
    test("should provide accurate queue statistics", async () => {
      ...
      // Wait for completion
      await waitFor(
        () => queue.getStats().active === 0 && queue.getStats().pending === 0,
      );
