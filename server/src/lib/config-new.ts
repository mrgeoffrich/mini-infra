...
const appConfig: Config = {
  server: {
    nodeEnv: getConfigValue('server.nodeEnv', 'NODE_ENV', 'development') as 'development' | 'production' | 'test',
  ...
  auth: {
    google: {
      clientId: getConfigValue('auth.google.clientId', 'GOOGLE_CLIENT_ID', null),
      clientSecret: getConfigValue('auth.google.clientSecret', 'GOOGLE_CLIENT_SECRET', null),
    },
    session: {
      secret: getConfigValue('auth.session.secret', 'SESSION_SECRET', null),
    },
    apiKey: {
      secret: getConfigValue('auth.apiKey.secret', 'API_KEY_SECRET', 'default-secret-change-in-production'),
    },
  },
  logging: {
    level: getConfigValue('logging.level', 'LOG_LEVEL', 'info') as 'trace' | 'debug' | 'info' | 'warn' | 'error' | 'fatal' | 'silent',
  },
  docker: {
    containerCacheTtl: getConfigValue('docker.containerCacheTtl', 'CONTAINER_CACHE_TTL', 3000),
    containerPollInterval: getConfigValue('docker.containerPollInterval', 'CONTAINER_POLL_INTERVAL', 5000),
  },
  azure: {
    apiTimeout: getConfigValue('azure.apiTimeout', 'AZURE_API_TIMEOUT', 15000),
  },
  connectivity: {
    checkInterval: getConfigValue('connectivity.checkInterval', 'CONNECTIVITY_CHECK_INTERVAL', 300000),
  },
};
