 ...
 */
router.get('/cloudflare', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const metadata = typeof latestStatus.metadata === 'string' 
    ...
    const response = {
      ...
      responseTime: latestStatus.responseTimeMs ? Number(latestStatus.responseTimeMs) : null
    };
 ...
 */
router.get('/cloudflare/history', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const transformedHistory = history.map((item: any) => {
      ...
      return {
        ...
        responseTime: item.responseTimeMs ? Number(item.responseTimeMs) : null
      };
    });
